window.onload = function () {
    v1 = new Vue({
        ...
        data: {
            ...
            //金额相关: 按月查看-图表数据
            money_monthData: {
                columns: ['costDate', 'costMoney'],
                rows: [
                    {'costDate': '2019年06月01日', 'costMoney': 0.00}	//测试数据
                ]
            },
            ...
            money_weekData: {
                columns: ['costDate', 'costMoney'],
                rows: [
                    {'costDate': '2019年01周星期日', 'costMoney': 0.00},
                    {'costDate': '2019年01周星期一', 'costMoney': 0.00},
                    {'costDate': '2019年01周星期二', 'costMoney': 0.00},
                    {'costDate': '2019年01周星期三', 'costMoney': 0.00},
                    {'costDate': '2019年01周星期四', 'costMoney': 0.00},
                    {'costDate': '2019年01周星期五', 'costMoney': 0.00},
                    {'costDate': '2019年01周星期六', 'costMoney': 0.00}
                ]
            },
            ...
            money_totalMonthCost: 0.00,				//按月查看-合计金额
            ...
            money_totalWeekCost: 0.00,			//按周查看-合计金额
            ...
            //笔数相关: 按月查看-图表数据
            num_monthData: {
                columns: ['costDate', 'costNum'],
                rows: [
                    {'costDate': '2019年06月01日', 'costNum': 0}		//测试数据
                ]
            },
            ...
            num_weekData: {
                columns: ['costDate', 'costNum'],
                rows: [
                    {'costDate': '2019年01周星期日', 'costNum': 9},
                    {'costDate': '2019年01周星期一', 'costNum': 3},
                    {'costDate': '2019年01周星期二', 'costNum': 4},
                    {'costDate': '2019年01周星期三', 'costNum': 5},
                    {'costDate': '2019年01周星期四', 'costNum': 1},
                    {'costDate': '2019年01周星期五', 'costNum': 4},
                    {'costDate': '2019年01周星期六', 'costNum': 8}
                ]
            },
            ...
            num_totalMonthCost: 0,			//按月查看-合计笔数
        ...
        methods: {
            ...
            init_moneyWeekData: function (t_year, t_week) {
                ...
                var temp = '';
                for (var i = 0; i < 7; i++) {
                    temp = this.money_weekData.rows[i].costDate.split("周")[1];
                    this.money_weekData.rows[i].costDate = t_year + "年" + t_week + "周" + temp;
                    this.money_weekData.rows[i].costMoney = 0.00;
                }
            },
            ...
            init_numWeekData: function (t_year, t_week) {
                ...
                var temp = '';
                for (var i = 0; i < 7; i++) {
                    temp = this.num_weekData.rows[i].costDate.split("周")[1];
                    this.num_weekData.rows[i].costDate = t_year + "年" + t_week + "周" + temp;
                    this.num_weekData.rows[i].costNum = 0;
                }
            },
            ...
            findMoneyByDate: function (t_date) {
                ...
                axios.get(this.url + "cost/findByDateBetween", {
                    ...
                    handle.money_totalMonthCost = 0.00;					//按月查看-合计金额
                    ...
                    for (var i = 0; i < dayNum; i++) {
                        ...
                        for (var j = 0; j < response.data.length; j++) {
                            if (response.data[j].costDate.indexOf(t_string) >= 0) {
                                handle.money_monthData.rows[i].costMoney = handle.money_monthData.rows[i].costMoney + response.data[j].costMoney;
                                t_totalMoney = t_totalMoney + response.data[j].costMoney;

                            }
                        }
                    }
                    handle.money_totalMonthCost = t_totalMoney;
                }).catch(function (error) {
                    console.log(error);
                });
            },
            ...
            findMoneyByWeek: function (t_weekDate) {
                ...
                axios.get(this.url + "cost/findByDateBetween", {
                    ...
                    handle.money_totalWeekCost = 0.00;						//按周查看-合计金额
                    ...
                    for (var i = 0; i < response.data.length; i++) {
                        temp1 = response.data[i].costDate.split("-")[0];
                        temp2 = response.data[i].costDate.split("-")[1];
                        temp3 = response.data[i].costDate.split("-")[2];
                    ...
                    for (var i = 0; i < 7; i++) {
                        for (var j = 0; j < tempArr.length; j++) {
                            if (tempArr[j].indexOf(handle.money_weekData.rows[i].costDate) >= 0) {
                                handle.money_weekData.rows[i].costMoney = handle.money_weekData.rows[i].costMoney + response.data[j].costMoney;
                                t_totalMoney = t_totalMoney + response.data[j].costMoney;
                            }
                        }
                    }
                    handle.money_totalWeekCost = t_totalMoney;
                }).catch(function (error) {
                    console.log(error);
                });
            },
            ...
            findNumByDate: function (t_date) {
                ...
                axios.get(this.url + "cost/findByDateBetween", {
                    ...
                    handle.num_totalMonthCost = 0;						//按月查看-合计笔数
                    ...
                    for (var i = 0; i < dayNum; i++) {
                        ...
                        for (var j = 0; j < response.data.length; j++) {
                            if (response.data[j].costDate.indexOf(t_string) >= 0) {
                                handle.num_monthData.rows[i].costNum = handle.num_monthData.rows[i].costNum + 1;
                                t_totalNum = t_totalNum + 1;
                            }
                        }

                    }
                    handle.num_totalMonthCost = t_totalNum;
                }).catch(function (error) {
                    console.log(error);
                });
            },
            ...
            findNumByWeek: function (t_weekDate) {
                ...
                axios.get(this.url + "cost/findByDateBetween", {
                    ...
                    handle.num_totalWeekCost = 0;							//按周查看-合计笔数
                    ...
                    for (var i = 0; i < 7; i++) {
                        for (var j = 0; j < tempArr.length; j++) {
                            if (tempArr[j].indexOf(handle.num_weekData.rows[i].costDate) >= 0) {
                                handle.num_weekData.rows[i].costNum = handle.num_weekData.rows[i].costNum + 1;
                                t_totalNum = t_totalNum + 1;
                            }
                        }
                    }

                    handle.num_totalWeekCost = t_totalNum;
                }).catch(function (error) {
                    console.log(error);
                });
            },
            ...
            loading: function () {
                ...
                this.money_totalMonthCost = 0.00;					//按月查看-合计金额
