window.onload = function () {
    v1 = new Vue({
        ...
        data: {
            ...
            //金额相关: 按月查看-图表数据
            money_monthData: {
                columns: ['rebateDate', 'rebateMoney'],
                rows: [
                    {'rebateDate': '2019年06月01日', 'rebateMoney': 0.00}	//测试数据
                ]
            },
            ...
            money_weekData: {
                columns: ['rebateDate', 'rebateMoney'],
                rows: [
                    {'rebateDate': '2019年01周星期日', 'rebateMoney': 0.00},
                    {'rebateDate': '2019年01周星期一', 'rebateMoney': 0.00},
                    {'rebateDate': '2019年01周星期二', 'rebateMoney': 0.00},
                    {'rebateDate': '2019年01周星期三', 'rebateMoney': 0.00},
                    {'rebateDate': '2019年01周星期四', 'rebateMoney': 0.00},
                    {'rebateDate': '2019年01周星期五', 'rebateMoney': 0.00},
                    {'rebateDate': '2019年01周星期六', 'rebateMoney': 0.00}
                ]
            },
            ...
            money_totalMonthRebate: 0.00,		//按月查看-合计金额
            ...
            money_totalWeekRebate: 0.00,		//按周查看-合计金额
            ...
            //笔数相关: 按月查看-图表数据
            num_monthData: {
                columns: ['rebateDate', 'rebateNum'],
                rows: [
                    {'rebateDate': '2019年06月01日', 'rebateNum': 0}		//测试数据
                ]
            },
            ...
            num_weekData: {
                columns: ['rebateDate', 'rebateNum'],
                rows: [
                    {'rebateDate': '2019年01周星期日', 'rebateNum': 9},
                    {'rebateDate': '2019年01周星期一', 'rebateNum': 3},
                    {'rebateDate': '2019年01周星期二', 'rebateNum': 4},
                    {'rebateDate': '2019年01周星期三', 'rebateNum': 5},
                    {'rebateDate': '2019年01周星期四', 'rebateNum': 1},
                    {'rebateDate': '2019年01周星期五', 'rebateNum': 4},
                    {'rebateDate': '2019年01周星期六', 'rebateNum': 8}
                ]
            },
            ...
            num_totalMonthRebate: 0,		//按月查看-合计笔数
            ...
            num_totalWeekRebate: 0,		//按周查看-合计笔数
        ...
        methods: {
            ...
            init_moneyWeekData: function (t_year, t_week) {
                ...
                var temp = '';
                for (var i = 0; i < 7; i++) {
                    temp = this.money_weekData.rows[i].rebateDate.split("周")[1];
                    this.money_weekData.rows[i].rebateDate = t_year + "年" + t_week + "周" + temp;
                    this.money_weekData.rows[i].rebateMoney = 0.00;
                }
            },
            ...
            init_numWeekData: function (t_year, t_week) {
                ...
                var temp = '';
                for (var i = 0; i < 7; i++) {
                    temp = this.num_weekData.rows[i].rebateDate.split("周")[1];
                    this.num_weekData.rows[i].rebateDate = t_year + "年" + t_week + "周" + temp;
                    this.num_weekData.rows[i].rebateNum = 0;
                }
            },
            ...
            findMoneyByDate: function (t_date) {
                ...
                axios.get(this.url + "rebate/findByDateBetween", {
                    ...
                    handle.money_totalMonthRebate = 0.00;					//按月查看-合计金额
                    ...
                    for (var i = 0; i < dayNum; i++) {
                        ...
                        for (var j = 0; j < response.data.length; j++) {
                            if (response.data[j].consumeDate.indexOf(t_string) >= 0) {
                                handle.money_monthData.rows[i].rebateMoney = handle.money_monthData.rows[i].rebateMoney + response.data[j].consumeRebatePrice;
                                t_totalMoney = t_totalMoney + response.data[j].consumeRebatePrice;

                            }
                        }
                    }
                    handle.money_totalMonthRebate = t_totalMoney;
                }).catch(function (error) {
                    console.log(error);
                });
            },
            ...
            findMoneyByWeek: function (t_weekDate) {
                ...
                axios.get(this.url + "rebate/findByDateBetween", {
                    ...
                    handle.money_totalWeekRebate = 0.00;						//按周查看-合计金额
                    ...
                    for (var i = 0; i < response.data.length; i++) {
                        temp0 = response.data[i].consumeDate.split(" ")[0];
                        temp1 = temp0.split("-")[0];
                        temp2 = temp0.split("-")[1];
                        temp3 = temp0.split("-")[2];
                    ...
                    for (var i = 0; i < 7; i++) {
                        for (var j = 0; j < tempArr.length; j++) {
                            if (tempArr[j].indexOf(handle.money_weekData.rows[i].rebateDate) >= 0) {
                                handle.money_weekData.rows[i].rebateMoney = handle.money_weekData.rows[i].rebateMoney + response.data[j].consumeRebatePrice;
                                t_totalMoney = t_totalMoney + response.data[j].consumeRebatePrice;
                            }
                        }
                    }
                    handle.money_totalWeekRebate = t_totalMoney;
                }).catch(function (error) {
                    console.log(error);
                });
            },
            ...
            findNumByDate: function (t_date) {
                ...
                axios.get(this.url + "rebate/findByDateBetween", {
                    ...
                    handle.num_totalMonthRebate = 0;						//按月查看-合计笔数
                    ...
                    for (var i = 0; i < dayNum; i++) {
                        ...
                        for (var j = 0; j < response.data.length; j++) {
                            if (response.data[j].consumeDate.indexOf(t_string) >= 0) {
                                handle.num_monthData.rows[i].rebateNum = handle.num_monthData.rows[i].rebateNum + 1;
                                t_totalNum = t_totalNum + 1;
                            }
                        }

                    }
                    handle.num_totalMonthRebate = t_totalNum;
                }).catch(function (error) {
                    console.log(error);
                });
            },
            ...
            findNumByWeek: function (t_weekDate) {
                ...
                axios.get(this.url + "rebate/findByDateBetween", {
                    ...
                    handle.num_totalWeekRebate = 0;						//按周查看-合计笔数
                    ...
                    for (var i = 0; i < 7; i++) {
                        for (var j = 0; j < tempArr.length; j++) {
                            if (tempArr[j].indexOf(handle.num_weekData.rows[i].rebateDate) >= 0) {
                                handle.num_weekData.rows[i].rebateNum = handle.num_weekData.rows[i].rebateNum + 1;
                                t_totalNum = t_totalNum + 1;
                            }
                        }
                    }

                    handle.num_totalWeekRebate = t_totalNum;
                }).catch(function (error) {
                    console.log(error);
                });
            },
            ...
            loading: function () {
                ...
                this.money_totalMonthRebate = 0.00;					//按月查看-合计金额
