/*global $, document
 *Update: JS/JavaScript中概念区分：global对象、window对象、document对象 
	js不仅局限在浏览器，globald是整个js的核心，在任何环境中都存在。但它本身不可以直接访问，只是一个概念。
 	windoww只是浏览器引擎上实现的一个对global对象的封装。它可以理解成浏览器的窗口。
	document是window对象的一个属性，是显示窗口内的一个文档。它可以理解成文档，就是网页。
 *global $，表示global javascript
*/



/*Update：jQuery简介
 *jQuery是以恶JavaScript库，它极大的简化了JavaScript编程。
 *jQuery中事件方法的一些例子
 ...
 *jQuery事件 - ready()方法
 	...
 	由于该事件在文档就绪后发生，因此把所有其它的jQuery事件和函数置于该事件中是非常好的做法。
...
// ------------------------------------------------------- //
// Charts Style Settings				图表样式设置
// ------------------------------------------------------ //

$(document).ready(function () {

    'use strict';				/*Update：use strict表示严格模式  */


    // ------------------------------------------------------- //
    // Charts Gradients			图表渐变
    // ------------------------------------------------------ //
